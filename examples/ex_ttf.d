...
enum MAX_RANGES = 256;
immutable(char)* font_file = "data/DejaVuSans.ttf";
...
{
    int[MAX_RANGES * 2] ranges;
    int count;
    int i;

    count = al_get_font_ranges(f, MAX_RANGES, ranges.ptr);
    writeln(count);
    for (i = 0; i < count; i++) {
...
static auto get_string(const char *key)
{
...
static void render()
{
    ...
    auto OFF = (int x) => al_ustr_offset(u, x);
    auto SUB = (int x, int y) => al_ref_ustr(&sub_info, u, OFF(x), OFF(y));
...
int main(string[] argv)
{
...

    {
        ...
        ex.f_alex = al_grab_font_from_bitmap(glyph, 1, ranges.ptr);
    }
